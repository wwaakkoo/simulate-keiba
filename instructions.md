...
- [x] データ取得コマンド (`collect_data.py`)
- [x] 2024年レースデータの収集 (288レース)

### Phase 2: 予測エンジン & API 🏗️
- [x] 脚質判定ロジック (`logic.py`)
- [x] 馬分析 API (`/api/analysis/horses/{id}`)
- [ ] 予測モデルの構築 (scikit-learn)

### Phase 3: レースシミュレーション（次に着手）
- [ ] PixiJS セットアップ & キャンバス表示
- [ ] トラック（コース）の描画
- [ ] 馬のスプライト表示
- [ ] レース展開ロジック（スタート〜ゴール）
- [ ] 実況風テキスト表示（オプション）
